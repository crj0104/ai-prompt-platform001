...
public class ApiController {
    ...
    public Map<String, Object> favorite(@PathVariable Long id) {
        return portalService.toggleFavorite(id);
    }

    @DeleteMapping("/templates/{id}/favorite")
    public Map<String, Object> unfavorite(@PathVariable Long id) {
        return portalService.removeFavorite(id);
    }

    @PostMapping("/orders")
    public Map<String, Object> purchase(@Valid @RequestBody PurchaseRequest request) {
        return portalService.purchaseTemplate(request);
    }

    @PostMapping("/templates/{id}/use")
    public Map<String, Object> useTemplate(@PathVariable Long id, @Valid @RequestBody UseTemplateRequest request) {
        return portalService.useTemplate(id, request.getInputSummary());
    }

    @PostMapping("/reviews")
    public Map<String, Object> review(@Valid @RequestBody ReviewRequest request) {
        return portalService.submitReview(request);
    }

    @GetMapping("/stats/creator")
    public Map<String, Object> creatorStats() {
        return portalService.creatorDashboard();
    }
